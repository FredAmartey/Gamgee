...
export async function POST(req: Request) {
  ...
  const parsed = z
    .object({
      ...
      // Long, spec-like directions (layout, per-element colours, "do not add X")
      // are followed well by the current image models, so the cap is generous.
      // It exists to bound the request, not to shape the writing.
      // A short key, not a raw model id: the mapping lives server-side so a
      // caller can't point this at an arbitrary (or expensive) model.
      imageModel: z
        .enum(["auto", "flash", "gemini-pro", "gpt-image", "flux"])
        .optional(),
  ...
  try {
    ...
    // "auto" keeps the routing above. Anything else is the user picking an
    // engine by hand from the sidebar; all four are live on Together and were
    // verified against a real logo prompt.
    const PICKABLE = {
      flash: "google/flash-image-3.1",
      "gemini-pro": "google/gemini-3-pro-image",
      "gpt-image": "openai/gpt-image-2",
      flux: "black-forest-labs/FLUX.2-pro",
    } as const;
    const picked =
      data.imageModel && data.imageModel !== "auto"
        ? PICKABLE[data.imageModel]
        : null;
    const model =
      picked ??
      (hasText ? "google/flash-image-3.1" : "black-forest-labs/FLUX.2-pro");
    const isFlux = model.startsWith("black-forest-labs/");

    const body = {
      prompt,
      model,
      // 2048, not 1024. The SVG export traces this image, and at 1024 there are
      // only ~60px of height per letter, which is too little for the tracer to
      // resolve a counter: letter bowls came out as mangled polygons. Doubling
      // it fixes that visibly. 4096 is better still but takes ~17s to trace in
      // the browser and 16x the pixels to generate, which isn't worth it.
      width: 2048,
      height: 2048,
      ...
      // `steps`) on Together; the other models reject it.
      ...(isFlux ? { guidance: 7 } : {}),
    };
    ...
    try {
      ...
      if (isFlux || isCallerError(primaryError)) throw primaryError;
    ...
    // Out of credit reads as a generic upstream failure otherwise, and the
    // "try again in a moment" copy sends you off retrying something that can
    // only be fixed by topping up.
    const outOfCredit = z
      .object({
        error: z.object({ error: z.object({ type: z.literal("credit_limit") }) }),
      })
      .safeParse(error);

    if (outOfCredit.success) {
      return new Response(
        "Your Together AI account is out of credit. Add funds at https://api.together.ai/settings/billing, then try again.",
